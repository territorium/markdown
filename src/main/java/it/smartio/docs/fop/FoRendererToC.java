...
import it.smartio.docs.fop.builder.FoPageNumberCitation;
...
class FoRendererToC implements Renderer, NodeVisitor<FoBlock> {
	 ...
	 * @param title
	 */
	public FoRendererToC(FoConfig data, String title) {
		this.data = data;
		this.title = title;
	}
	 ...
	 * Get the {@link FoRoot}.
	 */
	protected final FoConfig getData() {
		return this.data;
	}
	...
	public final void render(Book node) {
		...
		FoFlow flow = getData().createFlow(this.title, FoConfig.PAGES_STANDARD, true, properties);
		flow.setStartIndent("0pt").setEndIndent("0pt");
		flow.addBlock().setBreakAfter("page");

		String id = Long.toHexString(new Random().nextLong());
		getData().getRoot().addBookmark(id).setTitle(this.title);
