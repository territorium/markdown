...
/**
 * The {@link MarkdownReader} implements a reader based on Markdown. The Reader
 * starts from a file an includes referenced files to create a single huge
 * Markdown file.
 */
public class MarkdownReader {
	...
	private static final Pattern INCLUDE = Pattern.compile("^\\[([^\\]]*)\\]\\((.+\\.md)\\)", Pattern.CASE_INSENSITIVE);
	// Header definition: #+ HEADER
	private static final Pattern HEADER = Pattern.compile("^([#]+)\\s.+", Pattern.CASE_INSENSITIVE);
	// Footnote definition: [ID]: URL "TITLE"
	private static final Pattern FOOTNOTE = Pattern.compile("^\\[(\\w+)\\]:\\s([^\\s]+)(\\s\".+\")?",
			Pattern.CASE_INSENSITIVE);
	// Footnote reference: [TEXT]?[ID]
	private static final Pattern FOOTNOTE_REF = Pattern.compile("(?:\\[([^\\]]+)\\])\\[(\\w+)\\]",
			Pattern.CASE_INSENSITIVE);
